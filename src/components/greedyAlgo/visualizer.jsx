...
// FIX: previously, if `target` in the stored JSON was invalid (e.g. "abc"),
// the WHOLE case (including a perfectly valid array) was discarded by
// returning null. Now we keep the array and simply drop the bad target.
const parseStoredSearchCase = (input) => {
  ...
  try {
    ...
    if (parsed && Array.isArray(parsed.array)) {
      const cleanArray = parsed.array
        .map(Number)
        .filter((item) => !Number.isNaN(item))

      if (!cleanArray.length) return null
      ...
      if (Number.isNaN(target)) {
        // Invalid target, but array is still usable.
        return { array: cleanArray }
      }

      return { array: cleanArray, target }
    }
  } catch {
...
export default function Visualizer() {
  ...
  // FIX: centralised, robust target validation (handles '', null, undefined,
  // and non-numeric strings like "abc" which previously slipped through and
  // could be passed as NaN into the step generator).
  ...
  const handleSearch = () => {
    ...
    if (algorithm && algoMap[algorithm]) {
      ...
      if (generator) {
        clearPlayback()
        loadSteps(generator(baseArray, parsedTarget), {
          autoPlay: !isStepMode,
        })
      }
    }
  ...
  const getStateClass = (index) => {
    ...
    if (algorithm === 'linearSearch') {
      ...
      // NOTE: this assumes activeIndices is ordered as [mid, low, high],
      // matching the shape produced by binarySearchSteps.js. If that
      // generator's step shape changes, update the destructure here too.
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
                    ...
                    >
                      <button
                        ...
                        disabled={isRunning || !isTargetValid}
